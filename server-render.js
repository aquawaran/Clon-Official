...
// Импорт базы данных
const { initDatabase, migrateUsers, User, Post, Follow, Notification } = require('./database');
const crypto = require('crypto');

// Инициализация базы данных
initDatabase().then(() => {
  // Запускаем миграцию пользователей после инициализации БД
  migrateUsers();
}).catch(console.error);
...
// Удаление поста (только автор или создатель)
app.delete('/api/posts/:postId', authenticateToken, async (req, res) => {
    try {
        ...
        // Проверяем права: автор поста или создатель
        ...
        // Оповещаем автора поста (если удаляет не автор)
        if (!isAuthor) {
        ...
        // Трансляция удаления поста в реальном времени
...
app.post('/api/verification/request', authenticateToken, async (req, res) => {
    try {
        ...
        if (!request || request.trim().length < 10) {
            return res.status(400).json({ error: 'Заявка должна содержать минимум 10 символов' });
        }
        ...
        if (user.verification_request) {
            return res.status(400).json({ error: 'Вы уже подали заявку на верификацию' });
        }

        await User.requestVerification(user.user_id, request.trim());

        // Оповещаем создателя о новой заявке
        const creator = await User.findByUserId(CREATOR_ID);
        if (creator) {
            sendNotification(creator.id, {
                type: 'verification_request',
                message: `Новая заявка на верификацию от ${user.name}`,
                data: { userId: user.id, userName: user.name }
            });
        }

        res.json({ message: 'Заявка на верификацию подана' });
    } catch (error) {
        console.error('Ошибка подачи заявки на верификацию:', error);
        res.status(500).json({ error: 'Ошибка сервера' });
    }
});

// Получение списка заявок на верификацию (только для создателя)
app.get('/api/verification/requests', authenticateToken, checkCreator, async (req, res) => {
    try {
...
// Одобрение верификации (только для создателя)
app.post('/api/verification/approve/:userId', authenticateToken, checkCreator, async (req, res) => {
    try {
        ...
        await User.setVerification(user.user_id, true);

        // Оповещаем пользователя об одобрении
        sendNotification(user.id, {
            type: 'verification_approved',
            message: 'Поздравляем! Ваша верификация одобрена',
            data: {}
        });

        res.json({ message: 'Верификация одобрена' });
    } catch (error) {
...
// Отклонение верификации (только для создателя)
app.post('/api/verification/reject/:userId', authenticateToken, checkCreator, async (req, res) => {
    try {
        const { userId } = req.params;
        const user = await User.findById(userId);
        ...
        await User.rejectVerification(user.user_id);

        // Оповещаем пользователя об отклонении
        sendNotification(user.id, {
            type: 'verification_rejected',
            message: 'К сожалению, ваша заявка на верификацию отклонена',
            data: {}
        });

        res.json({ message: 'Верификация отклонена' });
    } catch (error) {
...
// Снятие верификации (только для создателя)
app.delete('/api/verification/:userId', authenticateToken, checkCreator, async (req, res) => {
    try {
        const { userId } = req.params;
        const user = await User.findById(userId);
        ...
        await User.setVerification(user.user_id, false);

        // Оповещаем пользователя о снятии верификации
        sendNotification(user.id, {
            type: 'verification_revoked',
            message: 'Ваша верификация была снята',
            data: {}
        });

        res.json({ message: 'Верификация снята' });
    } catch (error) {
    ...
    try {
        ...
        res.status(201).json({
            ...
            user: {
                ...
                is_verified: newUser.user_id === CREATOR_ID
            }
        });
    } catch (error) {
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (!userId) {
            ...
            await User.setUserId(user.id, userId);
        }
...
app.put('/api/profile', authenticateToken, async (req, res) => {
    try {
        ...
        if (currentUser.is_banned) {
            return res.status(403).json({ error: 'Забаненным пользователям запрещено изменять профиль' });
        }
...
app.post('/api/posts', authenticateToken, upload.array('media', 5), async (req, res) => {
    try {
        ...
        // Проверяем, не забанен ли пользователь
        if (user.is_banned) {
            return res.status(403).json({ error: 'Забаненным пользователям запрещено создавать посты' });
        }
...
app.post('/api/posts/:postId/comments', authenticateToken, async (req, res) => {
    try {
        ...
        // Проверяем, не забанен ли пользователь
        if (user.is_banned) {
            return res.status(403).json({ error: 'Забаненным пользователям запрещено комментировать' });
        }
