...
const User = {
  ...
  async getAll() {
    const result = await pool.query('SELECT id, name, username, avatar, bio, user_id, is_banned FROM users ORDER BY created_at DESC');
    return result.rows;
  },

  async getBanned() {
    const result = await pool.query('SELECT id, name, username, avatar, bio, user_id, is_banned FROM users WHERE is_banned = TRUE ORDER BY created_at DESC');
    return result.rows;
  },

  async searchAll(query) {
    const result = await pool.query(
      `SELECT id, name, username, avatar, bio, user_id, is_banned
       FROM users
